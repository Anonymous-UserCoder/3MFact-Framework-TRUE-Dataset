...
"""
    ...
    while new_key in existing_data:
        ...
        if "QA" in existing_data[new_key] and isinstance(existing_data[new_key]["QA"], dict):
            qa_content = existing_data[new_key]["QA"]
            if all(k in qa_content for k in ['Question', 'Answer', 'Confidence']):
                print(f"键 {new_key} 中的 QA 包含所有必要字段")
                counter += 1
                new_key = f"Follow_Up_Question_{counter}"
...
{{
  ...
  "Evidence Citation Guidelines": {{
    ...
    "2. Citation Format": "Use the format [Question_type Question_number_Query number_evidence number] for citations. For example:",
      "- Initial Question Generation": "[Initial_Question_Generation_Query X_evidenceY]",
      "- Follow Up Questions": "[Follow_Up_Question_Z_Query X_evidenceY]",
      "where X is the query number, Y is the evidence number, and Z is the follow-up question number.",
    ...
    "5. Output Inclusion": "When citing evidence in the final output, include the full citation tags in the 'Reasons' section of the Output_Answer_Format (e.g., [Follow_Up_Question_1_Query 1_evidence_2])."
  }}
}}
"""
